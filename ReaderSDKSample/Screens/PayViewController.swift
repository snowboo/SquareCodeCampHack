...
final class PayViewController: BaseViewController {
    ...
    private lazy var donateButton = PrimaryButton(title: "Round Up To \(format(amount: amount + (100 - amount%100))) To Donate", target: self, selector: #selector(donateButtonTapped))
    private lazy var checkoutButton = SecondaryButton(title: "Charge \(format(amount: amount))", target: self, selector: #selector(checkoutButtonTapped))
    ...
    private let amount = 227
    
    override func viewDidLoad() {
        super.viewDidLoad()
        let options = ["matcha latte", "hot chocolate", "mocha", "cappuchino", "custom"]
    ...
    @objc private func donateButtonTapped() {
        ...
        let donation = 100 - amount%100
        let money = SQRDMoney(amount: amount + donation)
